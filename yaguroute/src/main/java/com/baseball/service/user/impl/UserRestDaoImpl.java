...
public class UserRestDaoImpl implements UserRestDao {
	...
	public String getAccessTokenNaver(String authorizationCode) throws Exception {
		...
		// HTTP 요청 필요한 파라미터 설정
		String postParams = "grant_type=authorization_code" + "&client_id=" + "SLklwWhn8OnVgyPdEeSU" + "&redirect_uri="
				+ "kbo.yaguroute.online/user/naver" + "&code=" + authorizationCode + "&client_secret=" + "qV43i4IrXX";
	...
	public Map<String, Object> getNaverUserInfo(String accessToken) throws Exception {
		...
		System.out.println("userId : " + userId + " userNickName : " + userNickName + " userName : " + userName
				+ " userEmail : " + userEmail + " userImage : " + userImage + " userPhone : " + userPhone
				+ " userBirth : " + userBirth1 + userBirth2 + " gender : " + gender);
	...
	public Map<String, Object> getKakaoUserInfo(String accessToken) throws Exception {
		...
		if(kakaoAccount.get("gender").toString() == null && kakaoAccount.get("gender").toString().equals("")) {
	...
	/*
	 * @Override public String getUserImage(@ModelAttribute("file") MultipartFile
	 * file) throws Exception { // TODO Auto-generated method stub
	 * 
	 * if(file.isEmpty()) { return null; }
	 * 
	 * String filePath = null; String fileName = null;
	 * 
	 * try { //String absolutePath = new File("").getAbsolutePath(); fileName =
	 * StringUtils.cleanPath(file.getOriginalFilename()); //String uploadPath =
	 * absolutePath+fileUploadPath; filePath = fileUploadPath+fileName;
	 * 
	 ...
	 * 
	 * 
	 * // 파일 저장 //file.transferTo(new File(uploadPath + fileName));
	 * 
	 * 
	 * System.out.println("업로드 완료"); } catch (Exception e) { // TODO: handle
	 * exception e.printStackTrace(); }
	 * 
	 * // 데이터 베이스에 알맞게 저장하기 filePath = "/images/user/"+fileName;
	 * 
	 * 
